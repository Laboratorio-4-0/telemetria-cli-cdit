...
const { setApiKey } = useTrenLigeroStore()
const basicModal = useModalMessage()

async function verifyStatusDependencia() {
  notAvailable.value = await isDependenciaInactiva(ListaDependencias.TrenLigero)
}

async function VerifyMapboxApiKeyExist() {
  const response = await apiKeysService.getMapApiKeyByDependencia(ListaDependencias.TrenLigero)
